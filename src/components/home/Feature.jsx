...
export default function Feature({ isChromium }) {
  ...
  const x1 = useTransform(scrollYProgress, [0.25, 0.3], ["0px", "200px"]);
  ...
  const x = useTransform(scrollYProgress, [0.25, 0.3], ["15%", "-6%"]);
  const y = useTransform(scrollYProgress, [0.25, 0.3], ["-25%", "20%"]);
  const scale = useTransform(scrollYProgress, [0.25, 0.3], ["0.5", "0.9"]);

  // Sticky header
  const position = useTransform(scrollYProgress, (pos) => {
    return pos < 0.25 ? "relative" : "sticky";
  });

  // Gradient
  const xGradient = useTransform(scrollYProgress, [0.25, 0.3], ["0rem", "-10rem"]);
    ...
    <motion.section ref={targetRef} className={styles.feature}>
      <motion.div className={styles.wrapper} style={{ position }}>
        ...
        <div className={styles.main}>
          ...
          <motion.svg style={isChromium ? { x: xGradient, scaleGradient, filter: hueGradient } : { x: xGradient, scaleGradient, filter: "hue-rotate(10deg)" }} className={styles.gradient} xmlns="http://www.w3.org/2000/svg" width="1392" height="1346" viewBox="0 0 1392 1346" fill="none">
